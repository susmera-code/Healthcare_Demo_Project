...
const Patient = () => {
  ...
  const [apptCity, setApptCity] = useState("");
  const [apptState, setApptState] = useState("");
  const [apptPincode, setApptPincode] = useState("");
  // Booking state
  ...
  const handleConfirmBooking = async () => {
    ...
    if (!useRegisteredAddress) {
      if (!apptAddress || !apptCity || !apptPincode) {
    ...
    try {
      ...
      const finalAddress = useRegisteredAddress
        ? {
  ...
  return (
    <>
      <h1 className="text-blue fw-bold" style={{ padding: '1rem 1rem 1rem 9rem' }}>Welcome, {profile.full_name}</h1>

      <div className="container card">

        {/* Search Form */}
        <h4 className="text-blue fw-bold">Search Professionals</h4>
        <div className="mt-4">
        ...
        <div class="table-responsive w-100">
          <table className="table table-bordered align-middle mt-4">
            ...
            <tbody>
              {results.length === 0 ? <tr><td colSpan="8" className="text-center text-muted">No results found</td></tr> :
                results.map((p, i) => (
                  <tr key={p.id}>
                    <td>{i + 1}</td>
                  ...
                  <div className="d-flex flex-column">
                    ...
                    <div className="mt-3">
                      <label className="mb-2 d-block">
                        Is address same as registered address?
                      </label>
                        ...
                        <label>
                          <input
                            ...
                            onChange={() => {
                              ...
                              setApptAddress(profile?.address || "");
                              setApptCity(profile?.city || "");
                              setApptState(profile?.state || "");
                              setApptPincode(profile?.pincode || "");
                            }}
                          />{" "}
                          Yes
                        </label>
                      ...
                      {/* If NO → show reusable component */}
                      {!useRegisteredAddress && (
                        <AddressFields
                          prefix="appt"
                          address={apptAddress}
                          setAddress={setApptAddress}
                          city={apptCity}
                          setCity={setApptCity}
                          pincode={apptPincode}
                          setPincode={setApptPincode}
                          hideState={true}
                        />
                      )}
                    </div>
