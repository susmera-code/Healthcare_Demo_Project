...
const generatePaymentInvoice = (appt) => {
  ...
  doc.text(
    `Professional charges for the services of Nursing/Physiotherapy`,
    20,
    cursorY
  );
  ...
  invoiceCounter = invoiceCounter ? parseInt(invoiceCounter) + 1 : 1;

  // Save updated counter
  localStorage.setItem("invoiceCounter", invoiceCounter);

  // Format counter as 3 digits
  const formattedCounter = String(invoiceCounter).padStart(3, "0");
  ...
  // Final filename
  const invoiceFileName = `Suba_${formattedCounter}/${month}/${year}.pdf`;
